...
motionTime = []                                 # A new list 'time' for storing
                                                # the time when movement detected

# Initialising DataFrame variable 'dataFrame' using pandas
# libraries panda with Initial and Final column
dataFrame = panda.DataFrame(
                columns=["Initial", "Final"]
)

video = cv2.VideoCapture(0)                     # starting the webCam to capture
                                                # the video using cv2 module

# using infinite loop to capture the frames from the video
while True:

    check, cur_frame = video.read()             # Reading each image or frame from
                                                # the video using read function

    var_motion = 0                              # Defining 'motion' variable
                                                # equal to zero as initial frame
